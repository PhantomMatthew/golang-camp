...
func Decoder(msg []byte) (*GoimPacket, error) {
	if len(msg) < getProtocolHeaderSize()+1 {
		return nil, ErrorBrokenPacket
	}

	packageLength := binary.BigEndian.Uint32(msg[:PACKAGE_LENGTH_DATA_SIZE])

	headerLength := binary.BigEndian.Uint16(msg[PACKAGE_LENGTH_DATA_SIZE : PACKAGE_LENGTH_DATA_SIZE+HEADER_LENGTH_DATA_SIZE])

	protocolVersion := binary.BigEndian.Uint16(msg[PACKAGE_LENGTH_DATA_SIZE+HEADER_LENGTH_DATA_SIZE : PACKAGE_LENGTH_DATA_SIZE+HEADER_LENGTH_DATA_SIZE+PROTOCOL_VERSION_DATA_SIZE])

	operationCode := binary.BigEndian.Uint32(msg[PACKAGE_LENGTH_DATA_SIZE+HEADER_LENGTH_DATA_SIZE+PROTOCOL_VERSION_DATA_SIZE : PACKAGE_LENGTH_DATA_SIZE+HEADER_LENGTH_DATA_SIZE+PROTOCOL_VERSION_DATA_SIZE+OPERATION_CODE_DATA_SIZE])

	sequenceId := binary.BigEndian.Uint32(msg[PACKAGE_LENGTH_DATA_SIZE+HEADER_LENGTH_DATA_SIZE+PROTOCOL_VERSION_DATA_SIZE+OPERATION_CODE_DATA_SIZE : PACKAGE_LENGTH_DATA_SIZE+HEADER_LENGTH_DATA_SIZE+PROTOCOL_VERSION_DATA_SIZE+OPERATION_CODE_DATA_SIZE+SEQUENCE_ID_DATA_SIZE])

	content := msg[getProtocolHeaderSize():]
	return &GoimPacket{
